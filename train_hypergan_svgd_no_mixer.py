...
class HyperGANTrainer(object):
    ...
    def train(self):
        ...
        for epoch in range(1000):
            for batch_idx, (data, target) in enumerate(self.data_train):
                ...
                codes = torch.randn(5, self.batch_size, self.z).to(self.device)
                ...
                if batch_idx % 100 == 0:
                    ...
                    print ('**************************************')
                    print ('{}-no_m train, epoch: {}'.format(args.dataset, epoch))
    ...
    def test(self, ens_size, voting='hard'):
        test_acc = 0.
        test_loss = 0.
        total_correct = 0.
        ...
        for i, (data, target) in enumerate(self.data_test):
            codes = torch.randn(5, ens_size, self.z).to(self.device)
            params = self.hypergan.generator(codes)
