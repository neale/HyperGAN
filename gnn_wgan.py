...
def sample_z(args, v=False):
    z = torch.randn(args.batch_size, args.dim).cuda()
    z = autograd.Variable(z, volatile=v)
    return z


def sample_z_like(shape, v=False):
...
def train(args):
    ...
    optimizerE = optim.Adam(netE.parameters(), lr=1e-4, betas=(0.5, 0.999), weight_decay=1e-4)
    optimizerW1 = optim.Adam(W1.parameters(), lr=1e-4, betas=(0.5, 0.999), weight_decay=1e-4)
    optimizerW2 = optim.Adam(W2.parameters(), lr=1e-4, betas=(0.5, 0.999), weight_decay=1e-4)
    optimizerW3 = optim.Adam(W3.parameters(), lr=1e-4, betas=(0.5, 0.999), weight_decay=1e-4)
    optimizerD = optim.Adam(netD.parameters(), lr=1e-4, betas=(0.5, 0.999), weight_decay=1e-4)
    ...
    for _ in range(1000):
        for batch_idx, (data, target) in enumerate(mnist_train):
            ...
            for i in range(5):
                ...
                w1_code = sample_z_like((args.batch_size, 32, 32), v=True)
                w2_code = sample_z_like((args.batch_size, 32, 32), v=True)
                w3_code = sample_z_like((args.batch_size, 32, 32), v=True)
                w1_out, w2_out = [], []
                for i in range(32):
                    w1_out.append(W1(w1_code[i]))
                    w2_out.append(W2(w2_code[i]))
                l1 = autograd.Variable(torch.stack(w1_out).data)
                l2 = autograd.Variable(torch.stack(w2_out).data)
                l3 = autograd.Variable(W3(w3_code.contiguous().view(args.batch_size, -1)).data)
            ...
            for (z1, z2, z3) in zip(l1, l2, l3):
                ...
                d1.backward(one, retain_graph=True)
                optimizerW1.step()
                optimizerW2.step()
                optimizerW3.step()
            ...
            if batch_idx % 50 == 0:
                ...
                print ('Best test loss: ', best_loss, 'Best test acc: ', best_acc)
            ...
            if batch_idx % 200 == 0:
                ...
                (y_acc, y_loss), _ = utils.test_samples(args, [z1, z2, z3], train=True)
                print ("Loss: {}, Acc: {} ".format(test_loss, acc))
                print ('FT-Acc: ', y_acc, 'FT-Loss: ', y_loss)
                if acc > best_acc or (acc == best_acc and test_loss < best_loss):
                    best_acc = acc
                    if test_loss < best_loss:
                        best_loss = test_loss
                    #utils.save_model(args, W1, optimizerW1, 'wgan')
                    #utils.save_model(args, W2, optimizerW2, 'wgan')
                    #utils.save_model(args, W3, optimizerW3, 'wgan')
                    #utils.save_model(args, netD, optimizerD, 'wgan')
...
def test(args):
    ...
    optimizerW1 = optim.Adam(W1.parameters(), lr=5e-3, betas=(0.5, 0.999), weight_decay=1e-4)
    optimizerW2 = optim.Adam(W2.parameters(), lr=5e-3, betas=(0.5, 0.999), weight_decay=1e-4)
    optimizerW3 = optim.Adam(W3.parameters(), lr=5e-3, betas=(0.5, 0.999), weight_decay=1e-4)
    optimizerD = optim.Adam(netD.parameters(), lr=5e-4, betas=(0.5, 0.999), weight_decay=1e-4)
    ...
    W1, optimizerW1 = utils.load_model(args, W1, optimizerW1, 'wgan')
    W2, optimizerW2 = utils.load_model(args, W2, optimizerW2, 'wgan')
    W3, optimizerW3 = utils.load_model(args, W3, optimizerW3, 'wgan')
    netD, optimizerD = utils.load_model(args, netD, optimizerD, 'wgan')
    for i in range(100):
        acc, zacc = 0., 0.
        test_loss, ztest_loss = 0., 0.
        for i, (data, y) in enumerate(mnist_test):
