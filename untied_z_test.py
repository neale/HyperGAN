...
def train(args):
    ...
    for _ in range(1000):
        for batch_idx, (data, target) in enumerate(mnist_train):
            ...
            z1 = sample_z_like(x.shape)
            z2 = sample_z_like(x2.shape)
            z1 = netGc(netE(z))
            z2 = netGc(netE(z))
            ...
            scaled_loss = (100*loss) + t2_loss + t1_loss
            ...
            if batch_idx % 500 == 0:
                ...
                for i, (data, y) in enumerate(mnist_test):
                    ...
                    z1_loss = F.mse_loss(z1, x)
                    z2_loss = F.mse_loss(z2, x2)
                    correct, loss = train_clf(args, [z1_fake, z2_fake], data, y, val=True)
                    acc += correct
