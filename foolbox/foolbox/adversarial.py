...
from .models import PyTorchModel, TensorFlowModel, KerasModel
...
class Adversarial(object):
    ...
    def _set_hypermodel(self, hypermodel, arch, sample_fn, model_fn):
        self.__hypermodel = hypermodel
        self.__target_arch = arch
        self._sample_fn = sample_fn
        self._model_fn = model_fn

    def _new_model(self):
        if self.__hypermodel is None:
            print ("Hypermodel mode not initialized, call _set_hypermodel first")
            return

        new_model = self._sample_fn(self.__hypermodel, self.__target_arch)
        fmodel = self._model_fn(new_model)
        self.__model = fmodel
