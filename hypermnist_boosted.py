...
def load_args():
    ...
    parser.add_argument('--boost', default=1000, type=int)
...
# hard code the two layer net
def boosted_test_clf(args, nets, x_dist, data, target):
    """ calc classifier loss """
    data, target = data.cuda(), target.cuda()
    outputs = []
    for module in nets:
...
def train(args):
    ...
    for _ in range(1000):
        for batch_idx, (data, target) in enumerate(mnist_train):
            ...
            nets = (netE, W1, W2, W3)
            correct, loss = train_clf(args, [l1, l2, l3], data, target)
            ...
            if batch_idx > 1 and batch_idx % 199 == 0:
                ...
                for i, (data, y) in enumerate(mnist_test):
                    min_loss_batch = 10.
                    correct, loss = boosted_test_clf(args, nets, x_dist, data, y)
                    test_acc += correct.item()
                    test_loss += loss.item()
                test_loss /= len(mnist_test.dataset)
                test_acc /= len(mnist_test.dataset)
                #acc, loss, model = save_test(args, nets)
